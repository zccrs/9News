...
{
    return proxy->view()->page()->currentFrame()->evaluateJavaScript(scriptSource);
}

void QWebViewItem::setScrollPosition(QPoint point)
{
    proxy->view()->page()->currentFrame()->setScrollPosition(point);
}

void QWebViewItem::addToJavaScriptWindowObject(const QString &name, QObject *object)
{
    proxy->view()->page()->currentFrame()->addToJavaScriptWindowObject(name, object);
}
